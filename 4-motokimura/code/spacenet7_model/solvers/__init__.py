...
from .losses import BinaryFocalLoss, CombinedLoss
...
def get_loss(config):
    ...
    def _get_loss(config, loss_name):
        if loss_name == 'bce':
            ...
            return BinaryFocalLoss(gamma=config.SOLVER.FOCAL_LOSS_GAMMA)
        else:
            raise ValueError()
    ...
    return CombinedLoss(loss_modules, config.SOLVER.LOSS_WEIGHTS)
