...
def label_mask(pred,  main_threshold=0.4, seed_threshold=0.75, w_pixel_t=50, pixel_t=90):
    ...
    av_pred = pred
    av_pred = av_pred[..., 0] * (1 - 0.7 * av_pred[..., 2]) * (1 -  0.2 * av_pred[..., 1])
    ...
    nucl_msk = (255 - pred[..., 0] * 255)
    nucl_msk = nucl_msk.astype('uint8')
    y_pred = watershed(nucl_msk, y_pred, mask=(pred[..., 0] > main_threshold), watershed_line=True, compactness=3)
