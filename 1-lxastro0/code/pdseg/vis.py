...
def visualize(cfg,
    ...
    color_map = get_color_map_list(cfg.DATASET.NUM_CLASSES ** 2
                                   if cfg.DATASET.NUM_CLASSES ** 2 < 256 else 256)
    ...
    def exe_run():
        if cfg.DATASET.INPUT_IMAGE_NUM == 1:
            ...
            for img1s, img2s, grts, img1_names, img2_names, valid_shapes, org_shapes in test_reader:
                ...
                pred, = exe.run(
                    program=test_prog,
                    feed={'image1': img1s, 'image2': img2s},
                    fetch_list=fetch_list,
                    return_numpy=True)
    ...
    for pred, pred_shape, grts, img_names, valid_shapes, org_shapes in exe_run():
        ...
        pred1, pred2 = pred[:idx], pred[idx:]  # fluid.layers.split(pred, 2, dim=0)
        ...
        for i in range(num_imgs):
            ...
            if cfg.DATASET.INPUT_IMAGE_NUM == 1 or \
                ...
                if cfg.VIS.RAW_PRED:
                    ...
                    np.save(vis_fn.replace(".png", ".npy"), res_map.astype(np.float32)) 
                else:
                    if cfg.VIS.ADD_LABEL:
                        grt_im = cv2.resize(grts[i], pred_shape, interpolation=cv2.INTER_NEAREST)
            ...
            if log_writer is not None:
                ...
                # Original image
                # BGR->RGB
                img = cv2.imread(os.path.join(cfg.DATASET.DATA_DIR, img_name))[..., ::-1]
                ...
                if grt is not None:
                    ...
                    log_writer.add_image("Label/{}".format(img_name), grt, epoch)
