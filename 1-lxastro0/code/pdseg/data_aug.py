...
def resize(img1, img2=None, grt1=None, grt2=None, mode=ModelPhase.TRAIN):
    ...
    if cfg.AUG.AUG_METHOD == 'unpadding':
        ...
        if grt1 is not None:
            grt1 = cv2.resize(grt1, target_size, interpolation=cv2.INTER_NEAREST)
        if grt2 is not None:
            grt2 = cv2.resize(grt2, target_size, interpolation=cv2.INTER_NEAREST)
        ...
        if mode == ModelPhase.TRAIN:
            ...
            scale_factor = get_random_scale(min_scale_factor, max_scale_factor, step_size)
            img1, img2, grt1, grt2 = randomly_scale_image_and_label(img1, img2, grt1, grt2, scale=scale_factor)
        ...
        if grt1 is not None:
            grt1 = cv2.resize(grt1, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
        if grt2 is not None:
            grt2 = cv2.resize(grt2, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
    else:
...
def randomly_scale_image_and_label(img1, img2=None, grt1=None, grt2=None, scale=1.0):
    """
    ...
    img1 = cv2.resize(img1, (new_width, new_height), interpolation=cv2.INTER_LINEAR)
    if img2 is not None:
        img2 = cv2.resize(img2, (new_width, new_height), interpolation=cv2.INTER_LINEAR)
    if grt1 is not None:
        grt1 = cv2.resize(grt1, (new_width, new_height), interpolation=cv2.INTER_NEAREST)
    if grt2 is not None:
        grt2 = cv2.resize(grt2, (new_width, new_height), interpolation=cv2.INTER_NEAREST)

    return img1, img2, grt1, grt2


def random_rotation(crop_img1, crop_img2, crop_seg1, crop_seg2, rich_crop_max_rotation, mean_value):
    """
    随机旋转图像和标签图
...
def rand_scale_aspect(crop_img1, crop_img2, crop_seg1, crop_seg2,
                      rich_crop_min_scale=0, rich_crop_aspect_ratio=0):
    """
    ...
    if rich_crop_min_scale == 0 or rich_crop_aspect_ratio == 0:
        ...
        for i in range(0, 10):
            ...
            if dh < img_height and dw < img_width:
                ...
                crop_img1 = cv2.resize(crop_img1, (img_width, img_height), interpolation=cv2.INTER_LINEAR)
                if crop_img2 is not None:
                    crop_img2 = cv2.resize(crop_img2, (img_width, img_height), interpolation=cv2.INTER_LINEAR)
                crop_seg1 = cv2.resize(crop_seg1, (img_width, img_height), interpolation=cv2.INTER_NEAREST)
                if crop_seg2 is not None:
                    crop_seg2 = cv2.resize(crop_seg2, (img_width, img_height), interpolation=cv2.INTER_NEAREST)
...
def hsv_color_jitter(crop_img1, crop_img2,
...
def rand_crop(crop_img1, crop_img2, crop_seg1, crop_seg2, mode=ModelPhase.TRAIN):
    """
    ...
    if not ModelPhase.is_train(mode):
        if crop_height < img_height or crop_width < img_width:
            raise Exception("Crop size({},{}) must large than img size({},{}) when in EvalPhase.".format(
    ...
    if img_height == crop_height and img_width == crop_width:
        ...
        if pad_height > 0 or pad_width > 0:
            crop_img1 = cv2.copyMakeBorder(crop_img1, 0, pad_height, 0, pad_width,
                                           cv2.BORDER_CONSTANT, value=cfg.DATASET.PADDING_VALUE)
            if crop_img2 is not None:
                crop_img2 = cv2.copyMakeBorder(crop_img2, 0, pad_height, 0, pad_width,
                                               cv2.BORDER_CONSTANT, value=cfg.DATASET.PADDING_VALUE)
            if crop_seg1 is not None:
                crop_seg1 = cv2.copyMakeBorder(crop_seg1, 0, pad_height, 0, pad_width,
                                               cv2.BORDER_CONSTANT, value=cfg.DATASET.IGNORE_INDEX)
            if crop_seg2 is not None:
                crop_seg2 = cv2.copyMakeBorder(crop_seg2, 0, pad_height, 0, pad_width,
                                               cv2.BORDER_CONSTANT, value=cfg.DATASET.IGNORE_INDEX)
        ...
        if crop_height > 0 and crop_width > 0:
            ...
            crop_img1 = crop_img1[h_off:(crop_height + h_off), w_off:(w_off + crop_width), :]
            if crop_img2 is not None:
                crop_img2 = crop_img2[h_off:(crop_height + h_off), w_off:(w_off + crop_width), :]
            if crop_seg1 is not None:
                crop_seg1 = crop_seg1[h_off:(crop_height + h_off), w_off:(w_off + crop_width)]
            if crop_seg2 is not None:
                crop_seg2 = crop_seg2[h_off:(crop_height + h_off), w_off:(w_off + crop_width)]
        return crop_img1, crop_img2, crop_seg1, crop_seg2
