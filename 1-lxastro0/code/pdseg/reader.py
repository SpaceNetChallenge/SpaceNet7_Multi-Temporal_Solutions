...
class SegDataset(object):
            ...
            if cfg.DATASET.INPUT_IMAGE_NUM == 1:
                ...
                if is_test:
                    ...
                    for img1, img2, lab, ig in reader():
                        ...
                        bs += 1
                        if bs == batch_size:
                            yield np.array(img1s), np.array(img2s), np.array(labs), np.array(ignore)
                            bs = 0
                            img1s, img2s, labs, ignore = [], [], [], []
    ...
    def load_image(self, line, src_dir, mode=ModelPhase.TRAIN):
        ...
        if img1 is None:
            raise Exception(
                "Empty image, src_dir: {}, img: {}".format(src_dir, img1_path))
        if len(img1.shape) < 3:
        ...
        if img2_name is not None:
            ...
            if img2 is None:
                raise Exception(
                    "Empty image, src_dir: {}, img: {}".format(src_dir, img2_path))
            if len(img2.shape) < 3:
                img2 = cv2.cvtColor(img2, cv2.COLOR_GRAY2BGR)
            if img1.shape != img2.shape:
                raise Exception("source img1 and source img2 must has the same size")
        else:
            img2 = None
        ...
        if grt1_name is not None:
            ...
            if grt1 is None:
                raise Exception(
                    "Empty image, src_dir: {}, label: {}".format(src_dir, grt1_path))
        ...
        if grt2_name is not None:
            ...
            if grt2 is None:
                raise Exception(
                    "Empty image, src_dir: {}, label: {}".format(src_dir, grt2_path))
        ...
        if img_channels != cfg.DATASET.DATA_DIM:
            raise Exception("Input image channel({}) is not match cfg.DATASET.DATA_DIM({}), img_name={}".format(
                img_channels, cfg.DATASET.DATADIM, img1_name))
        if img_channels != len(cfg.MEAN):
            raise Exception("img name {}, img chns {} mean size {}, size unequal".format(
                img1_name, img_channels, len(cfg.MEAN)))
        if img_channels != len(cfg.STD):
            raise Exception("img name {}, img chns {} std size {}, size unequal".format(
    ...
    def test_aug(self, img1):
        ...
        for ops in cfg.TEST.TEST_AUG_FLIP_OPS:
            if ops[0] == 'h':
                ...
                ret = np.concatenate((ret, np.transpose(img1, (1, 0, 2))), axis=2)
            else:
                ret = np.concatenate((ret, np.transpose(np.rot90(img1, k=2), (1, 0, 2))), axis=2)
    ...
    def process_image(self, line, data_dir, mode):
        """ process_image """
        img1, img2, grt1, grt2, img1_name, img2_name, grt1_name, grt2_name = self.load_image(line, data_dir, mode=mode)
        grt1 = grt1 + 1 if grt1 is not None else None
        if mode == ModelPhase.TRAIN:
            img1, img2, grt1, grt2 = aug.resize(img1, img2, grt1, grt2, mode)
            img1, img2, grt1, grt2 = aug.rand_crop(img1, img2, grt1, grt2, mode=mode)
                ...
                if cfg.AUG.RICH_CROP.BLUR:
                    ...
                    if n > 0:
                        if np.random.randint(0, n) == 0:
                            ...
                            if radius > 9:
                                radius = 9
                            img1 = cv2.GaussianBlur(img1, (radius, radius), 0, 0)
                            if img2 is not None:
                                img2 = cv2.GaussianBlur(img2, (radius, radius), 0, 0)
            ...
            img1, img2, grt1, grt2 = aug.resize(img1, img2, grt1, grt2, mode=mode)
            img1, img2, grt1, grt2 = aug.rand_crop(img1, img2, grt1, grt2, mode=mode)
        ...
        if grt2 is not None:
            ...
            ignore_idx = np.where((grt1 == cfg.DATASET.IGNORE_INDEX) | (grt2 == cfg.DATASET.IGNORE_INDEX))
        ...
        if cfg.DATASET.INPUT_IMAGE_NUM == 1:
            ...
            if ModelPhase.is_train(mode):
                ...
                return (img1, img2, grt, img1_name, img2_name, valid_shape, org_shape)
