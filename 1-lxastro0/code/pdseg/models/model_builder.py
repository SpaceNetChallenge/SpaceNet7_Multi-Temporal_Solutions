    ...
    for i, logits in enumerate(logits_split[1:]):
        if type(ops[i]) is str:
            if ops[i][0] == 'h':
                ...
                rot90 = fluid.layers.flip(fluid.layers.transpose(logits, [0, 1, 3, 2]), [2])
                rot180 = fluid.layers.flip(fluid.layers.transpose(rot90, [0, 1, 3, 2]), [2])
                logits_split[i + 1] = fluid.layers.transpose(rot180, [0, 1, 3, 2])
        else:
            times = (360 - ops[i]) // 90
            for _ in range(times):
                logits = fluid.layers.flip(fluid.layers.transpose(logits, [0, 1, 3, 2]), [2])
            logits_split[i + 1] = fluid.layers.transpose(logits, [0, 1, 3, 2])

    logits = fluid.layers.stack(logits_split, axis=0)  # channel_mul, batch_size, 1, h, w
...
def build_model(main_prog, start_prog, phase=ModelPhase.TRAIN):
    ...
    if ModelPhase.is_eval(phase) and cfg.TEST.TEST_AUG:
        channel_mul = len(cfg.TEST.TEST_AUG_FLIP_OPS) + len(cfg.TEST.TEST_AUG_ROTATE_OPS) + 1
    ...
    with fluid.program_guard(main_prog, start_prog):
        with fluid.unique_name.guard():
            ...
            image1 = fluid.data(name='image1', shape=image1_shape, dtype='float32')
            image2 = None
            if cfg.DATASET.INPUT_IMAGE_NUM == 2:
                 image2_shape = [-1, cfg.DATASET.DATA_DIM * channel_mul, height, width]
                 image2 = fluid.data(name='image2', shape=image2_shape, dtype='float32')
            ...
            if cfg.DATASET.INPUT_IMAGE_NUM == 1:
                if cfg.TEST.TEST_AUG:
                    image_split = fluid.layers.split(image1, channel_mul, dim=1)  # batch_size, 3 x channel_mul, h, w
                    image1 = fluid.layers.concat(image_split, axis=0)             # channel_mul * batch_size, 3, h, w
                    logits = seg_model(image1, class_num)
                    logits_split = fluid.layers.split(logits, channel_mul, dim=0)
                ...
                if cfg.TEST.TEST_AUG:
                    ...
                    logits1_split = fluid.layers.split(logits1, channel_mul, dim=0)
                    logits1 = test_aug_inv(logits1_split)

                    image_split = fluid.layers.split(image2, channel_mul, dim=1)  # batch_size, 3 x channel_mul, h, w
                    image2 = fluid.layers.concat(image_split, axis=0)  # channel_mul * batch_size, 3, h, w
                    logits2 = seg_model(image2, class_num)
                    logits2_split = fluid.layers.split(logits2, channel_mul, dim=0)
