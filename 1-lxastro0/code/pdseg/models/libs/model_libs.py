...
def fuse(input, num_classes, filter_size1=3, filter_size2=3, num_filters1=512, num_filters2=256):
    param_attr = fluid.ParamAttr(
        name='logit_weights',
        regularizer=fluid.regularizer.L2DecayRegularizer(
            regularization_coeff=0.0),
    ...
    fuse1 = conv_bn_layer(
        input, filter_size1, num_filters1, stride=1, padding=1, name="fuse1", if_act=False)
    fuse1 = fluid.layers.relu(fuse1)

    fuse2 = conv_bn_layer(
        fuse1, filter_size2, num_filters2, stride=1, padding=1, name="fuse2", if_act=False)
    fuse2 = fluid.layers.relu(fuse2)

    logit = conv(
        fuse2, num_classes, 3, stride=1, padding=1, bias_attr=True, param_attr=param_attr)
