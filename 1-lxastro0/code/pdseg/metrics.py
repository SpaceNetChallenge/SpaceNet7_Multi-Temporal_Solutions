...
class ConfusionMatrix(object):
    ...
    def __init__(self, num_classes=2, streaming=False):
        self.confusion_matrix = np.zeros([num_classes, num_classes], dtype=np.float)
    ...
    def zero_matrix(self):
        ...
        self.confusion_matrix = np.zeros([self.num_classes, self.num_classes], dtype=np.float)
    ...
    def accuracy(self):
        ...
        """
        pa = np.diag(self.confusion_matrix) / np.sum(self.confusion_matrix, axis=1)
        mpa = np.mean(pa)
        return pa, mpa
    ...
    def kappa(self):
        ...
        pe = np.dot(np.sum(self.confusion_matrix, axis=0), np.sum(self.confusion_matrix, axis=1)) / (cm_sum ** 2)
        kappa = (po - pe) / (1 - pe)
        return kappa
