...
def check_ious(poly1, area1, center1, poly2s, area2s, center2s, thr, center_thr, eps=1e-5):
...
def filter_countours(polygons, filter_polygons, margin):
    ...
    for idx, p in enumerate(polygons):
        if p.is_valid:
            ...
            if check_ious(p, area, center, filter_valid_polygons, filter_areas, filter_centers, thr=0, center_thr=5):
                filter_res[idx] = True
...
def process(npy_list, thres1=0.1, thres2_h1=0.6, thres2_l1=0.4, thres2_h2=0.6, thres2_l2=0.35,
    ...
    contours, polygons = get_building_polygon(npy_mean, thres2_h1, thres2_l1,
