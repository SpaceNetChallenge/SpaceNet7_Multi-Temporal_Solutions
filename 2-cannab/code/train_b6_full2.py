...
def train_epoch(current_epoch, combo_loss, model, optimizer, scaler, train_data_loader):
    ...
    for i, sample in enumerate(iterator):
        with torch.cuda.amp.autocast():
            ...
            loss = loss2  + loss1 * 0.4 + 0.4 * loss3
...
if __name__ == '__main__':
    ...
    best_snapshot_name = 'eff6_4k_{0}_best_full2_0'.format(fold)
    last_snapshot_name = 'eff6_4k_{0}_last_full2_0'.format(fold)

    np.random.seed(1231 + fold)
    random.seed(1231 + fold)
    ...
    data_train = TrainDataset(train_files, train_dir, masks_dir, crop_size=928, scale=4)
    ...
    # snap_to_load = 'eff7_4k_{0}_last_new_0'.format(fold)
    # if args.local_rank == 0:
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[100, 120, 140, 160], gamma=0.5)
