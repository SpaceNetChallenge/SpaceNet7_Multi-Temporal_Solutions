...
if __name__ == '__main__':
    ...
    best_snapshot_name = 'eff7_4k_{0}_best_double_0'.format(fold)
    last_snapshot_name = 'eff7_4k_{0}_last_double_0'.format(fold)
    ...
    data_train = TrainDatasetDouble(train_files, train_dir, masks_dir, aoi_list, crop_size=576, scale=4, tune=True)
    ...
    model = EfficientNet_Unet_Double(name='efficientnet-b7', pretrained=False)
    ...
    snap_to_load = 'eff7_4k_{0}_best_tuned_0'.format(fold)
    if args.local_rank == 0:
